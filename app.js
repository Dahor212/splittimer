...
function renderRouteDetail(){
  ...
  $('#routeStats').textContent = `${distTxt}  •  ${diff}  •  ${ascTxt}`;
  ...
  top5List.innerHTML = '';
  if (!top5.length){
    top5List.innerHTML = `<div class="hint subtle">Zatím žádné výsledky. Po první jízdě se zde objeví Top 5.</div>`;
  } else {
    top5.forEach((row, i)=>{
      ...
      el.innerHTML = `
        <div class="l">
          <div class="t">#${i+1}  ${escapeHtml(who)}</div>
  ...
  last3List.innerHTML = '';
  if (!last3.length){
    last3List.innerHTML = `<div class="hint subtle">Zatím žádné výsledky.</div>`;
  } else {
    last3.forEach((r)=>{
  ...
  if (!route.checkpoints.length){
    cpList.innerHTML = `<div class="hint subtle">Přidej checkpointy (vrchol, sjezd, zatáčka…). Za jízdy se budou brát v pořadí.</div>`;
  } else {
...
function renderRide(){
  ...
  const isFinishNext = (nextIdx >= route.checkpoints.length);
  if (!isFinishNext){
    ...
    const kmTxt = km!=null ? ` • ${String(km).replace('.',',')} km` : '';
    $('#rideNextLabel').textContent = `Další: Cíl${kmTxt}`;
    $('#btnNextCheckpoint').textContent = 'CÍL – Ukončit';
  }
  ...
  $('#btnNextCheckpoint').disabled = !state.ride.running;
